...
plt_barcoderanks <-
  ...
  scale_y_continuous(limits=c(1, 5000), breaks=c(1, 5, 10, 50, 100, 500, 1000, 4000), trans='log10', labels = scales::number)+
  labs(title=ggp_title,
       x="Cell barcodes sorted by number of counts [descending]",
       y="Total UMI count for each barcode") +
  ggp_theme_bw_square_01

ggsave("./04_dropseq_files/barcoderanks.png", plt_barcoderanks, height=20, width=20, units="cm")
...
write.table(tbl_emptydrops, "./04_dropseq_files/tbl_emptydrops.tsv", quote=F, sep="\t", row.names=T, col.names=NA)
...
ggsave("./04_dropseq_files/plt_vln_pre_emptydrops.png", plt_vln_pre_emptydrops, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_post_emptydrops.png", plt_vln_post_emptydrops, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_filter01.png", plt_vln_filter01, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_filter02.png", plt_vln_filter02, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_filter03.png", plt_vln_filter03, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_filter04.png", plt_vln_filter04, height=12, width=18, units="cm")
...
ggsave("./04_dropseq_files/plt_vln_filter.png", plt_vln_filter, height=12, width=18, units="cm")
...
write.table(summary_stat, "./04_dropseq_files/summary_stat.tsv", quote=F, sep="\t", row.names = F)
...
plt_umap <-
  ...
  geom_text_repel(data=df_umap %>% group_by(color) %>% summarise(q1=quantile(umap_1, 0.5), q2=quantile(umap_2, 0.5)),
                  aes(x=q1, y=q2, label=paste(LETTERS[1:num_cluster], summary(so_pc27@active.ident), sep="_")), size=6) +
  labs(title=ggp_title,
       x="UMAP_1",
       y="UMAP_2") +
...
ggsave("./04_dropseq_files/umap.png", plt_umap, height=15, width=15, units="cm")
