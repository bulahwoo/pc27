---
title: "scRNA-seq: pc27mitonows26_gtf"
author: "Andrea Elizabeth Acurio Armas, Bulah Wu, Petr Nguyen"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r barcode_geometry, echo=FALSE, fig.cap="RNAdia barcode geometry", out.width = '60%', fig.align = 'center'}
knitr::include_graphics("data/RNAdia_barcode_geometry.png")
```

According to the manual of RNAdia 2.0 kit (Dolomite Bio), the barcode sequence is located on Read 1, including a fragment of 2-nt "wobble bases", 12-nt cell barcode (CB) and 14-nt unique molecular identifier (UMI). Before constructing the count matrix, Read 1 raw reads were trimmed to keep only the 26-nt barcode fragment. The matrix was then built according to the Drop-seq core computational protocol developed by the McCarroll Lab (https://github.com/broadinstitute/Drop-seq).

```{bash, engine.opts='-l', echo=FALSE, comment=""}
cat /media/nguyen/Data1/mao/scseq/dropseq/pc27mitonows26_gtf/sh_dropseq_pc27mitonows26_gtf.sh
```
...
```{r knee-plot, warning=FALSE, eval=TRUE}
...
```{r barcodeRanks, warning=FALSE, eval=TRUE}
...
```{r prefilter, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r violin1, echo=FALSE, fig.width=12, fig.height=4, eval=TRUE}
...
```{r postfilter, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r violin2, echo=FALSE, fig.width=12, fig.height=4, eval=TRUE}
...
```{r umap, warning=FALSE, message=FALSE, eval=TRUE}
so_pc27 <- CreateSeuratObject(counts = mtx_pc27, min.cells = 3, min.features = 200, project = "pc27") %>%
           PercentageFeatureSet(pattern = "^agat|^rrn", col.name = "percent.mt") %>%
           subset(subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5) %>%
...
```{r marker01, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r marker02, echo=FALSE, fig.width=16, fig.height=4, eval=TRUE}
...
```{r marker04, echo=FALSE, fig.width=16, fig.height=4, eval=TRUE}
